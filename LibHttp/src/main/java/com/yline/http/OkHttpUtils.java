...
public class OkHttpUtils {
	...
	public static final int HANDLE_ERROR_CODE = -200; // 字符串处理出问题（Json解析）
	...
	private static <T> void requestAsync(final OnHttpAdapter httpAdapter, final OnJsonCallback<T> callback) {
		...
		okHttpClient.newCall(request).enqueue(new Callback() {
			...
			public void onFailure(Call call, IOException e) {
				LogUtil.v("requestAsync onFailure");
			...
			public void onResponse(Call call, Response response) throws IOException {
				...
				httpAdapter.handleResponse(response, dataType, new OnParseCallback<T>() {
					...
					public void onResponseSuccess(T t) {
						LogUtil.v("requestAsync onResponseSuccess");
						response(callback, t);
					}
					
					@Override
					public void onResponseError(Exception ex, int code, String msg) {
						LogUtil.v("requestAsync onResponseError code = " + code + ", msg = " + msg);
	 ...
	 * @param <T>      返回的泛型结构
	 */
	private static <T> void failure(final OnJsonCallback<T> callback, final OnHttpAdapter httpAdapter, final Exception ex, final int code, final String msg) {
